...
class Settings:
    ...
    def from_env(cls):
        ...
        try:
            ...
            raise RuntimeError("OWNER_ID harus berupa angka Telegram numeric user ID.")
        if owner_id <= 0:
            raise RuntimeError("OWNER_ID harus berupa angka positif.")

        db = os.getenv("DATABASE_URL", "").strip()
        if not db:
            raise RuntimeError("DATABASE_URL belum diisi. Bot ini menggunakan PostgreSQL.")
        ...
        if not db.startswith("postgresql+asyncpg://"):
            raise RuntimeError("DATABASE_URL harus berupa URL PostgreSQL Railway.")
