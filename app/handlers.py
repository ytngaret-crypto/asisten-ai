...
async def dot_commands(message: Message):
    ...
    if command == ".assistant":
        return await do_assistant(message, arg)
...
async def do_assistant(message, prompt):
    if not await can_use(message.bot, message.chat.id, message.from_user.id, "assistant"):
        return await message.answer("🔒 Group Assistant belum dibuka untuk akunmu.")
    if not prompt and message.reply_to_message:
        prompt = message.reply_to_message.text or message.reply_to_message.caption or ""
    if not prompt:
        return await message.answer("Contoh: <code>.assistant bantu rangkum pembahasan grup</code>", parse_mode="HTML")

    group = await db.get_group(message.chat.id)
    ctx = await build_context(message.chat.id, message.reply_to_message)
    system = f"""Kamu adalah Group Assistant untuk grup Telegram.
{style_text(group.personality)}
Bantu anggota memahami, merangkum, menyusun, dan melanjutkan pembahasan grup.
Ikuti topik yang sedang dibahas dan gunakan konteks grup jika relevan.
...
{ctx}"""
    try:
        ...
        await safe_answer(message, result or "Maaf, saya belum bisa membantu.")
    except Exception as e:
        logger.exception("Group Assistant error")
        await message.answer(f"⚠️ Group Assistant gagal: {type(e).__name__}")
...
async def do_tts(message, text):
    ...
    try:
        ...
        # Telegram voice messages require an OGG/Opus audio stream.
        # Convert Gemini's WAV/PCM result to OGG/Opus and send it as a voice note.
        try:
            import subprocess
            ...
            ogg_data, stderr = await proc.communicate(audio)
            if proc.returncode != 0 or not ogg_data:
                raise RuntimeError((stderr or b"FFmpeg conversion failed").decode(errors="replace"))
        except Exception:
            logger.exception("TTS OGG/Opus conversion failed")
            return await message.answer("⚠️ Audio berhasil dibuat, tetapi gagal dikonversi menjadi voice note Telegram.")

        sent = await message.answer_voice(
            BufferedInputFile(ogg_data, filename=f"voice-{voice}.ogg"),
            caption=f"🔊 TTS • Suara: {voice}"
        )
        await db.track_bot_message(message.chat.id, sent.message_id)
    except Exception as e:
...
async def callbacks(call: CallbackQuery):
    ...
    if data == "menu:assistant":
        return await call.answer("Gunakan .assistant pertanyaan atau instruksi di chat.", show_alert=True)
    if data == "menu:search":
